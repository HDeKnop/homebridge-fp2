...
async function discoverFp2Once(host: string, timeoutMs: number, log: Logging, preferredDeviceId?: string): Promise<DiscoveredFp2 | null> {
  ...
  return new Promise<DiscoveredFp2 | null>(resolve => {
    ...
    // dnssd's Browser re-emits mDNS socket errors (e.g. ENETUNREACH while the
    // host has no IPv4 route yet at boot) as 'error' events, and hap-controller's
    // IPDiscovery attaches no listener — unhandled, they crash the whole child
    // bridge. Treat them as a failed round instead.
    const onError = (err: Error) => {
      log.debug(`[discovery] mDNS error: ${err.message}`);
      finish(null);
    };

    discovery.on('serviceUp', onUp);
    (discovery as unknown as NodeJS.EventEmitter).on('error', onError);
    try {
      discovery.start();
    } catch (err) {
    ...
    (discovery as unknown as { browser?: NodeJS.EventEmitter }).browser?.on('error', onError);
