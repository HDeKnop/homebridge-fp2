...
export class Fp2HapClient extends EventEmitter {
  ...
  /** Consecutive failed connect attempts, for log throttling. Reset on any
   *  successful connect (including recovery via a successful poll). */
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.logRetry(`[${this.cfg.name}] connection failed: ${(err as Error).message}`);
      this.consecutiveFailures++;
      if (this.consecutiveFailures === FAILURE_WARN_ATTEMPTS) {
        this.log.warn(
          `[${this.cfg.name}] still unable to connect after ${FAILURE_WARN_ATTEMPTS} attempts — ` +
            `retrying continues (backoff up to ${RECONNECT_MAX_MS / 1000}s) but further failures are logged at debug level until it recovers`
        );
      }
  ...
  /** Warn for the first few consecutive failures, debug thereafter — a
   *  persistently-offline FP2 shouldn't fill the log every backoff cycle. */
  private logRetry(msg: string): void {
    if (this.consecutiveFailures < FAILURE_WARN_ATTEMPTS) {
      this.log.warn(msg);
    } else {
      this.log.debug(msg);
  ...
  private async connectWithRecovery(allowRepair: boolean): Promise<void> {
    ...
    if (discovered) {
      ...
      this.logRetry(`[${this.cfg.name}] mDNS discovery did not surface ${this.cfg.host} within ${DISCOVERY_TIMEOUT_MS}ms`);
    }
    ...
    if (stored) {
      ...
      try {
        ...
        if (stale && allowRepair) {
          ...
          this.client = null;
          // Clear the record under its own file key — `stored` may have been
          // recovered via findByDeviceId from a file keyed by a different host
          // than the config's (the store saves under the resolved IP). Clearing
          // only cfg.host would leave the stale pairing in place forever.
          await this.store.clear(stored.host);
          if (stored.host !== this.cfg.host) {
  ...
  startPolling(intervalSeconds: number): void {
    ...
    // Schema enforces a minimum of 5s, but a hand-edited config could pass 0
    // and turn this into a hot HAP-request loop — clamp in code too.
    const safeSeconds = Math.max(5, intervalSeconds);
  ...
  private async pollOnce(): Promise<void> {
    ...
    try {
      ...
      if (!this.state.reachable) {
        this.state.reachable = true;
        // A successful poll is a full recovery — reset the backoff so the next
        // drop reconnects quickly instead of waiting out the old backed-off delay.
        this.reconnectDelay = RECONNECT_INITIAL_MS;
        this.consecutiveFailures = 0;
        this.emit('connected');
      }
