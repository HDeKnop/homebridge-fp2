...
Thanks for your interest! This plugin started as a personal beginner
project for one home's set of FP2s, built entirely with/through
[Claude Code](https://claude.com/claude-code). Outside contributions are
not just welcome but actively hoped for — especially firmware-specific
quirks, additional zone behaviours, or platform compatibility fixes. The
maintainer is a hobbyist and realistically can't support setups beyond
his own, so contributors who can test on other configurations are
especially valuable.
